...
void gpio_port_init(gpio_port_t p_port, gpio_pin_t p_pin, gpio_funct_t p_funct, gpio_data_dir_t p_data_dir, gpio_pin_state_t p_state)
{
    switch(p_port)
    {
        case gpio_port_dd:
        	...
        	//THIS IS CORRECT: BOTH PORT DD AND TC ARE INCORRECT I THINK BUT THEY WORK FOR NOW
        case gpio_port_ta:
			GPIO_PAR(gpio_port_ta) |= (((p_funct)&0x3) << (p_pin*2));
			GPIO_DDR(gpio_port_ta) |= (1 << p_pin);
			break;
    ...
    switch(p_funct)
    {
    	...
    	case gpio_funct_primary:
    		//Do nothing?
    		//THIS IS WRITEN ABOVE< WILL FIX LATER
    		break;
